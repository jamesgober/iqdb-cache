# iqdb-cache -- Engineering Directives
...
iqdb-cache is an in-process caching layer for hot vectors and search results. For large indexes that do not fit in RAM, a well-tuned cache turns most reads into memory reads. It wraps any `Index` as a `CachedIndex` and is disabled by default, so it is purely an opt-in optimization.
...
- Caching is an optimization: the database is correct with the cache disabled, which is the default.
- Result-cache invalidation is handled (TTL first) so a stale result is never returned after a mutation.
